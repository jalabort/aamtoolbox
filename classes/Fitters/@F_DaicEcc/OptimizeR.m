function [delta,c] = OptimizeR(obj,i,tex,~)
...
%   dt_dy = imfilter(dt_dy,H,'replicate');
  ...
  g = obj.tm{i}.ComputeDTDQPR(sum(obj.sm{i}.n_r),dt_dx,dt_dy,obj.dw_dr{i},t,tt);
  q = g' * g;
  inv_q = inv(q);
  ...
  delta_r = inv_q * sd_delta;
  
  delta_qp = zeros(obj.sm{i}.n_q+obj.sm{i}.n_p,1);
